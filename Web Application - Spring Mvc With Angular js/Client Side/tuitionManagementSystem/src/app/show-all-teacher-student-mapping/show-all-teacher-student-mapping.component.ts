...
@Component({
  ...
  styleUrls: ['./show-all-teacher-student-mapping.component.css',
...
export class ShowAllTeacherStudentMappingComponent implements OnInit {
  ...
  constructor(private _router : Router, private _teacherStudentService : TeacherStudentService) {
  ...
  deleteTeacherStudent(studentId : string, teacherId : string){
    this._teacherStudentService.deleteTeacherStudent(studentId, teacherId).subscribe(
      (responseData) => {
